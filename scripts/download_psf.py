...
def get_names(args, logger):
    ...
    url ='https://cosmo.vera.psc.edu/Euclid-like-PSFs/monopsfs_euclidlike.zip'
    file_name = os.path.basename(url)
    target = os.path.join(target_dir, file_name)
    link_dir = os.path.join(share_dir, file_name)[:-len('.zip')]
    unpack_dir = target[:-len('.zip')]
...
def unpack(do_unpack, target, target_dir, unpack_dir, meta, args, logger):
    ...
    with zipfile.ZipFile(target, 'r') as zip_ref:
        ...
        def safe_extract(zip_ref, path=".", members=None):
            ...
            for member in zip_ref.namelist():
                if member != 'VIS_response.fits.gz': # not interested in downloading this file
