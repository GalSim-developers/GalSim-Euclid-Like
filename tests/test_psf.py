...
def test_psf_euclid():
    ...
    for i in range(len(psfobjs)):
        assert isinstance(psfobjs[i], galsim.Image) # check objects in list are Galsim Image objects
    return
...
def test_get_psf_function():
    ...
    np.testing.assert_allclose(psf_img.array, true_img.array, atol = 1e-5*np.sum(true_img.array),
